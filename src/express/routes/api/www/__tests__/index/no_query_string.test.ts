import { index } from '../../';

const spyConsoleError = jest.spyOn(global.console, 'error').mockImplementation();

jest.mock('@ogp/src/images/www/ogp_630x630.png', () => {
  return 'data:image/png;base64,';
});

describe('www/index', () => {
  beforeEach(() => {
    jest.resetModules();
  });

  test('query string指定がないときの確認', async () => {
    const resMock = {
      writeHead: jest.fn(),
      end: jest.fn(),
    };
    // @ts-ignore
    await index({ query: {} }, resMock);

    expect(1).toBe(spyConsoleError.mock.calls.length);
    expect([200, { 'Content-Type': 'image/png', 'Content-Length': 1797 }]).toEqual(
      resMock.writeHead.mock.calls[0],
    );
    expect(true).toBe(Buffer.isBuffer(resMock.end.mock.calls[0][0]));
  });
});
